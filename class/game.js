...
class EducationalPathGame {
    ...
    constructor(isBeingExtended = false) {
            ...
            // Фонова музика - список всіх доступних звуків
            // Використовуємо Cloudinary для великих файлів, локальні для малих
            ...
            // Створюємо об'єкти Audio для всіх треків
            this.backgroundMusicObjects = {};
            for (const [key, track] of Object.entries(this.backgroundMusicTracks)) {
                const audio = new Audio(track.file);
                audio.preload = 'auto';
                audio.loop = true;
                audio.volume = 0.05; // 5% гучності
                // Додаємо обробники помилок для Cloudinary
                audio.addEventListener('error', (e) => {
                    console.warn(`Помилка завантаження треку ${key} з ${track.file}:`, e);
                    // Можна додати fallback на локальний файл
                });
                this.backgroundMusicObjects[key] = audio;
            }
            
            // По замовчуванню запускається main_fon
            this.currentBackgroundMusicKey = 'main_fon';
            this.currentBackgroundMusic = this.backgroundMusicObjects['main_fon'];
    ...
            // Створюємо клітинки з mapData.js
        ...
        this.mapData.cells.forEach((cell, index) => {
            ...
            const special = this.specialCells[cell.id];
             ...
             // Додаємо кольоровий клас для маркування клітинок
             let colorClass = ''; // Нова змінна для кольорового класу
             if (special) {
                 switch(special.type) {
                     case 'pvp-quest':
                     case 'creative-quest':
                     ...
                     case 'webnovella-quest':
                         colorClass = 'cell-quest'; // Зелений
                         break;
                     case 'test-question':
                         colorClass = 'cell-test'; // Синій
                         break;
                     case 'reincarnation':
                     case 'early-reincarnation':
                         colorClass = 'cell-reincarnation'; // Чорний
                         break;
                     case 'amphitheater':
                         colorClass = 'cell-amphitheater'; // Коричневий
                         break;
                     case 'tavern':
                         colorClass = 'cell-tavern'; // Помаранчевий
                         break;
                     case 'casino':
                         colorClass = 'cell-casino'; // Червоний
                         break;
                     case 'alternative-path':
                         colorClass = 'alternative-path'; // Жовтий
                         break;
                     case 'machine-uprising':
                         // Залишаємо фінішний стиль
                         break;
                 }
             }
             
             // Додаємо базовий клас типу + кольоровий клас + клас зони
             cellElement.className = `board-cell ${cellClass} ${colorClass} zone-${zone}`;
    ...
    // Метод для зміни фонового треку
    setBackgroundTrack(trackKey) {
        if (!this.backgroundMusicObjects[trackKey]) {
            console.warn(`Трек ${trackKey} не знайдено`);
            return;
        }
        
        // Зберігаємо стан поточного треку перед зупинкою
        const wasPlaying = !this.currentBackgroundMusic.paused;
        
        // Зупиняємо поточний трек
        this.stopBackgroundMusic();
        
        // Встановлюємо новий трек
        this.currentBackgroundMusicKey = trackKey;
        this.currentBackgroundMusic = this.backgroundMusicObjects[trackKey];
        
        // Запускаємо новий трек, якщо музика була увімкнена
        if (wasPlaying) {
    ...
    switchBackgroundMusic() {
        // Перемикаємо на наступний трек у списку
        const trackKeys = Object.keys(this.backgroundMusicTracks);
        const currentIndex = trackKeys.indexOf(this.currentBackgroundMusicKey);
        const nextIndex = (currentIndex + 1) % trackKeys.length;
        this.setBackgroundTrack(trackKeys[nextIndex]);
    }
    
    setBackgroundVolume(volume) {
        // Встановлюємо гучність для всіх треків
        try {
            for (const audio of Object.values(this.backgroundMusicObjects)) {
                audio.volume = volume;
            }
        } catch (e) {
    ...
            // Визначаємо, чи це переможець (досяг 101 клітинки і зайняв перше місце)
            let backgroundImage = null;
            if (winner && winner.position >= 101) {
                // Перевіряємо, чи це перше місце
                const sortedPlayers = this.players.filter(p => !p.hasLost).sort((a,b) => {
                    if (b.position !== a.position) return b.position - a.position;
                    return b.points - a.points;
                });
                if (sortedPlayers.length > 0 && sortedPlayers[0].id === winner.id) {
                    // Це переможець - показуємо картинку winer
                    backgroundImage = 'image/modal_window/winer.jpg';
                    // Також показуємо картинку 101 для переможця
                    this.showQuestModal('Вітаємо!', 'Ви досягли клітинки 101!', [], 'image/modal_window/101.png');
                    // Після закриття показуємо вікно перемоги
                    setTimeout(() => {
    ...
        showQuestModalWithContent(html, setupCallback, backgroundImageUrl = null) {
    
            // Зберігаємо фонове зображення перед вставкою HTML
            const currentBackground = this.questModalContent.style.backgroundImage;
            const currentBackgroundSize = this.questModalContent.style.backgroundSize;
    ...
                    this.questModalContent.style.backgroundImage = currentBackground;
                    this.questModalContent.style.backgroundSize = currentBackgroundSize || 'cover';
                    this.questModalContent.style.backgroundPosition = currentBackgroundPosition || 'center';
                    this.questModalContent.style.backgroundRepeat = currentBackgroundRepeat || 'no-repeat';
                } else {
                    this.questModalContent.style.backgroundImage = 'none';
                }
            }
