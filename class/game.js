// game.js - НОВА, ВИПРАВЛЕНА ВЕРСІЯ (з мінімальними змінами)

class EducationalPathGame {
    constructor() {
        this.BOARD_SIZE = 101;
        // FIX: Додаємо новий параметр для розміру клітинок
        this.CELL_SIZE = 40; 
    ...
    // FIX: Зробили функцію асинхронною, щоб вона чекала створення карти
    async initializeGame() {
        ...
        // FIX: Тепер ми чекаємо, поки карта буде повністю готова
        ...
        if (startCell) this.centerViewOn(startCell);
    }
    
    // FIX: Нова функція, яка чекає на дані карти перед тим, як її малювати
    ...
    createBoard() {
        ...
        // Створюємо окрему стартову клітинку, щоб position: 0 працювало
        ...
        this.mapData.cells.forEach((cell) => {
            ...
            // FIX: Задаємо позицію та ЗБІЛЬШЕНИЙ РОЗМІР
    ...
    // FIX: Більш агресивна функція для видалення старих елементів
    removeOldElements() {
        const elementsToRemove = document.querySelectorAll('#game-board > div:not(#game-map)');
        elementsToRemove.forEach(el => {
             console.warn('Force removing potentially old element:', el);
             el.remove();
        });
    }
    ...
    createStaticZones() {
        ...
        svgContainer.style.cssText = `position: absolute; top: 0; left: 0; width: 100%; height: 100%; z-index: 1;`;
    ...
    // FIX: Оновлена функція для малювання чіткого шляху
    drawSequentialPath() {
        this.pathSvg.innerHTML = '';
        if (!this.mapData || this.mapData.cells.length < 1) return;

        // Лінія від СТАРТУ до першої клітинки
        const startEl = document.getElementById('cell-0');
        const firstCell = this.mapData.cells[0];
        let pathData = `M ${startEl.offsetLeft + startEl.offsetWidth / 2} ${startEl.offsetTop + startEl.offsetHeight / 2}`;
        pathData += ` L ${firstCell.x + this.CELL_SIZE / 2} ${firstCell.y + this.CELL_SIZE / 2}`;
        
        // Лінії між усіма іншими клітинками
        for (let i = 0; i < this.mapData.cells.length - 1; i++) {
            const next = this.mapData.cells[i+1];
            pathData += ` L ${next.x + this.CELL_SIZE / 2} ${next.y + this.CELL_SIZE / 2}`;
        }
        ...
        path.setAttribute('stroke-width', '5'); // Товста лінія
    ...
    async movePawnToCell(player, cellId) {
        return new Promise(resolve => {
            ...
            // FIX: Рухаємось відносно стартової клітинки, якщо cellId = 0
